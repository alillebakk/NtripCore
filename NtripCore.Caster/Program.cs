...
namespace NtripCore.Caster
{
    internal class Program
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            _logger.LogInformation("NTRIP Caster server is running. Press Ctrl+C to stop.");
            
            // Wait for the host to stop (this keeps the application alive)
            await host.RunAsync();

            // Stop the server when the host stops
            _logger.LogInformation("Stopping NTRIP Caster server...");
            caster.StopServer();
